...
{   
    ...
    ?><FONT size="7"color="#008000"><center>  <?php
    echo "<b>Bonne réponse </b><br> ";  ?>
    </center></FONT> <FONT color="#000000"><?php
  ...
  ?>
   ...
   echo "</tr>"; ?></FONT> <?php
    ...
    while ($ligne= $req3->fetch()) //les reponses de utilisateur 
    { 
...
else if  ($_SESSION['i']<11)
{   if($_SESSION['nquestion']<2)
{
    $_SESSION['nquestion']=$_SESSION['nquestion']+1;
}
    else{  $var_r= rand(1,6); 
        if (in_array( $var_r,$_SESSION["clientQCM"]))// test pour savoir si il y a question 
    {
    while (in_array( $var_r, $_SESSION["clientQCM"])==TRUE )
    {
        $var_r= rand(1,6); 
    }

        }    $_SESSION['iQ']=$_SESSION['iQ']+1;

$_SESSION["clientQCM"][$_SESSION['iQ']]= $var_r;
$_SESSION['nquestion']=0;
    }   
    ?>
    <form method="post" action="questionsuivanteRANDOM.php">
