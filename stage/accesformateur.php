 <!-- 1er page partie formateur 
 sur cette page nous pouvons voir historique -->

<?php
session_start();
$_SESSION['e-mail'] =$_POST['i_mail'] ;// verifier email  qui vien de connexionfor.php
$_SESSION['mp'] =$_POST['i_mp'] ;// verifier code  qui vien de connexionfor.php
...
<body>
<?php
   $var_t= 0; // verifie dans la base si oui ou non les identifant et mot de passe sont juste 
   $bdd= new PDO ('mysql:host=localhost;dbname=stage_s4','root','');

   // differente requete pour les differents cas 
   $req=$bdd->query("SELECT* FROM utilisateur ");
   $req1=$bdd->query("SELECT * FROM historique GROUP BY idhis ASC ");
   $re2=$bdd->query("SELECT * FROM historique GROUP BY score DESC ");
 ...
 // ---------verification du motdepasse et de e_mail ---------------------
 while ($ligne= $req->fetch())
     { 
      if($_SESSION['e-mail']==$ligne['e_mail'] AND$_SESSION['mp']==$ligne['mot_de_passe'] AND $ligne['formation']=='formateur')
          {
            $var_t= 1;
           }
